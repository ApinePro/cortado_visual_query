...
export class SequenceGroup extends VariantElement {
  ...
  public recalculateWidth(): number {
    ...
    this.width =
      this.elements
        .map((el: VariantElement) => el.getWidth())
        .reduce((a: number, b: number) => a + b) +
      2 * Constants.MARGIN_X +
      this.getHeadLength() -
...
export class ParallelGroup extends VariantElement {
  ...
  public recalculateHeight(): number {
    ...
    this.height =
      this.elements
        .map((el: VariantElement) => el.getHeight() + Constants.MARGIN_Y)
  ...
  public recalculateWidth(): number {
    ...
    let headLength = this.getHeadLength();
    this.width =
      Math.max(...this.elements.map((el: VariantElement) => el.getWidth())) +
      Constants.MARGIN_X +
      2 * headLength;
    return this.width;
  }
