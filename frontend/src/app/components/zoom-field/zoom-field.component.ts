...
export class ZoomFieldComponent implements OnInit, AfterViewInit {
  ...
  centerContent(animationDuration : number){
    ...
    const [translateX, translateY] = this.computeCenterOffsets ? this.computeCenterOffsets(svg) : [0,0]

    d3.select(this.content.nativeElement)
      .transition()
      .duration(animationDuration)
      .ease(d3.easeExpInOut)
      .call(this.zoom.transform, d3.zoomIdentity.translate(translateX, translateY));
  }

  focusSelected(animationDuration : number){

    const svg = d3.select(this.content.nativeElement);
    const [translateX, translateY] = this.computeFocusOffsets ? this.computeFocusOffsets(svg) : [0,0]
    svg
      .transition()
      .duration(animationDuration)
      .ease(d3.easeExpInOut)
