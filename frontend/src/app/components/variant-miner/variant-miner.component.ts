...
import { AfterViewInit, Component, ElementRef, Inject, OnChanges, OnInit, Renderer2, SimpleChanges } from '@angular/core';
  ...
  animations: [
    trigger('collapse', [
      ...
      transition(':leave', [
        animate(
          '250ms ease-in',
          style({opacity: '0.4', width : '0px'})
        ),
      ]),
    ]),
  ]
})
export class VariantMinerComponent extends LayoutChangeDirective implements OnInit, AfterViewInit, OnChanges{
  ...
  displayedVariantsPatterns : Array<SubvariantPattern> = new Array<SubvariantPattern>();
  ...
  handleFilterChange(event){
    ...
    this.displayedVariantsPatterns =  this.variantPatterns.filter((vp) => {
      ...
      if(vp.k >= this.kLow && vp.k <= this.kHigh &&
         ((this.showOnlyMaximal && vp.maximal) || !this.showOnlyMaximal) &&
         ((this.showOnlyClosed && vp.closed) || !this.showOnlyClosed) &&
          vp.support >= this.supLow && vp.support <= this.supHigh &&
          vp.child_parent_confidence >= this.cpConfLow && vp.child_parent_confidence <= this.cpConfHigh &&
          vp.cross_support_confidence >= this.crossConfLow && vp.cross_support_confidence <= this.crossConfHigh &&
          vp.subpattern_confidence >= this.spConfLow && vp.subpattern_confidence <= this.spConfHigh
         ){
  ...
  sortDisplayedVariants(key : VariantSortKey){
    ...
    this.displayedVariantsPatterns.sort((a : SubvariantPattern, b : SubvariantPattern) => {
      if (a[key] < b[key]) {
