...
export class DropZoneComponent extends DropZoneDirective implements OnInit {
  ...
  @Output() fileOnHover : EventEmitter<boolean> = new EventEmitter<boolean>(false);

  dropZoneDirective = DropZoneDirective;

  constructor(private sanitizer: DomSanitizer,
              private backendService : BackendService) {super()}

  ngOnInit() {
    this.dropZoneText = this.sanitizer.bypassSecurityTrustHtml(this.dropzoneConfig.dropZoneHoverMessage);
  }
  ...
  importDroppedFile(files: NgxFileDropEntry[]) {
    ...
    if(files.length > 1){
        ...
        Swal.fire({
          title: '<tspan class = "text-warning">Multiple Files not supported </tspan>',
        ...
        if(this.file.fileEntry.isFile){
          ...
          const fileEnding : string = fileName.slice(fileName.lastIndexOf("."), fileName.length);
          ...
          if(this.dropzoneConfig.acceptedFormats.includes(fileEnding)){
            ...
            fileEntry.file((file: File) => {
              switch(fileEnding){
                case ".xes": (!environment.electron) ? this.backendService.uploadEventLog(file) : this.backendService.loadEventLogFromFilePath("test");
                              break;

                case ".ptml": this.backendService.loadProcessTreeFromFilePath(file["path"]); break;
                default : break;
          ...
          Swal.fire({
            title:'<tspan class = "text-warning">Directory not supported</tspan>',
            html: '<b>Error Message: </b><br>' +
