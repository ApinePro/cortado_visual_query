...
export class TreeStringRendererComponent implements OnInit, OnChanges{
  ...
  colorActivityNames(value: any): string {
    ...
    knownActivities.forEach((activityName : string) => {
      value = value.replace(new RegExp(activityName, 'g'), `<b><span style="color:${this.activityColorMap.get(activityName)}">`+ activityName+'</span></b>')
    })

    unknowActivities.forEach((activityName : string) => {
      value = value.replace(new RegExp("'" + activityName + "'", 'g'), "'<span class=\"warning-highlight\">" + activityName + "</span>'")
    })
  ...
  replacePlaceHolderOperatorString(value : string): string {
    value = value.replace(/\*tau\*/g,   "<b>" + TAU_CHAR + "</b>")
