...
from api.routes.conformance.variantConformance import calculate_alignment_intern_with_timeout, get_alignment_callback
...
class WebSocketService:
    def __init__(self, websocket: WebSocket, callback: Callable, timeout: int, name: str):
        ...
        try:
            while True:
                ...
                try:
                    ...
                    print(f'sending response to ws {self.name}..')
...
class ConformanceWebsocketHandler(WebSocketService):
    def __init__(self, websocket: WebSocket, timeout: int):
        super().__init__(websocket, self.handle_message, timeout, 'conformance checking')
...
class ArcDiagramWebsocketHandler(WebSocketService):
    ...
    async def handle_message(self, data, timeout: int):
        self.pool.apply_async(
            ...
            callback=get_repetition_mining_callback(
                self.websocket
            ),
        )
...
async def websocket_conformance(websocket: WebSocket):
    ws_handler = ConformanceWebsocketHandler(websocket,
