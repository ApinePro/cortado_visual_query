...
def get_traces(variant: Tuple[Group, List[Trace], List, VariantInformation]) -> List[Trace]:
    return variant[1]


def get_variant_info(variant: Tuple[Group, List[Trace], List, VariantInformation]) -> List[Trace]:
    return variant[3]


def get_id_to_group_mapping(include_user_defined=False) -> Dict:
    variants: Mapping[int, Tuple[Group,
                                 Trace, List, VariantInformation]] = cache.variants
    variants = {k: v[0] for k, v in variants.items() if (
        v[3].is_user_defined and include_user_defined) or (not v[3].is_user_defined)}

    return variants


def get_variant_list(include_user_defined=False) -> List[Group]:
    return list(get_id_to_group_mapping(include_user_defined=include_user_defined).values())
...
def get_all_traces() -> List['Trace']:
    return list(itertools.chain(
        *[ts for _, (_, ts, _, _) in cache.variants.items()]))
